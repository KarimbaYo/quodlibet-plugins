...
class WaveformScale(Gtk.EventBox):
    ...
    def draw_waveform(
        ...
        if width == 0 or height == 0 or not self._rms_vals:
            return
        scale_factor = self.get_scale_factor()
        pixel_ratio = float(scale_factor)
        line_width = 1.0 / pixel_ratio
        ...
        max_value = max(self._rms_vals) if self._rms_vals else 1.0
        ...
        try:
            factor = CONFIG.compression_factor
            ...
            for x in range(
                ...
                if mouse_position >= 0:
                    if self._seeking:
                        ...
                        if x < mouse_position:
                            fg_color = elapsed_color if x < position_width else hover_color
                ...
                if factor != 1.0 and half_height > 0:
                    exponent = 1.0 / factor
                    normalized_val = val / half_height
                    val = (normalized_val ** exponent) * half_height

                if CONFIG.half_waveform:
                    cr.move_to(hx, height)
                    cr.line_to(hx, height - (val * 2))
                else:
    ...
    def draw_placeholder(self, cr, width, height, color: Gdk.RGBA):
        ...
        if CONFIG.half_waveform:
            baseline_y = height - (line_width / 2.0)

            cr.set_source_rgba(*self.elapsed_color(self.get_style_context()))
            cr.move_to(hw, baseline_y)
            cr.line_to(position_width, baseline_y)
            cr.stroke()
            cr.set_source_rgba(*color)
            cr.move_to(position_width, baseline_y)
            ...
            cr.set_source_rgba(*self.elapsed_color(self.get_style_context()))
            cr.rectangle(
                hw, half_height - line_width,
                position_width - hw, line_width * 2)
            cr.fill()
...
class WaveformSeekBarPlugin2(EventPlugin):
    """The plugin class."""

    PLUGIN_ID = "WaveformSeekBar2"
    PLUGIN_NAME = _("Waveform Seek Bar 2")
    ...
    def PluginPreferences(self, parent):
        ...
        def on_half_waveform_toggled(button, *args):
            CONFIG.half_waveform = button.get_active()
            if self._bar is not None:
                self._bar._waveform_scale.queue_draw()

        def on_compression_factor_changed(spinbox):
        ...
        sw = Gtk.Switch()
        label = Gtk.Label(_("Show only half waveform"))
        sw.set_active(CONFIG.half_waveform)
        sw.connect("notify::active", on_half_waveform_toggled)
        hbox = Gtk.HBox(spacing=6)
        hbox.pack_start(label, False, True, 0)
        hbox.pack_end(sw, False, True, 0)
        vbox.pack_start(hbox, True, True, 0)
        ...
        try:
            current_factor = CONFIG.compression_factor
        except (ValueError, TypeError):
            current_factor = 3.0

        hbox = Gtk.HBox(spacing=6)
        label = Gtk.Label(label=_("Compression factor (1=off):"))
        hbox.pack_start(label, False, True, 0)
        compression_spin = Gtk.SpinButton(
            adjustment=Gtk.Adjustment(current_factor, 1.0, 10.0, 0.1, 1.0, 0)
        )
        compression_spin.set_digits(1)
